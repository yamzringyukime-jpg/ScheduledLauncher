...
### 遅延時間の設定について
アプリごとの「遅延（秒）」設定は、**「そのアプリを起動してから、次のアプリを起動するまでの待ち時間（順次待機）」**を意味します。
- リストの1番目のアプリは、指定時刻になると即座に起動します。
- 1番目のアプリの遅延が10秒の場合、2番目のアプリはその10秒後に起動します。

## 注意事項

### PC時刻の手動変更について
アプリが起動中にPCのシステム時刻を手動で変更した場合、以下の意図しない挙動が発生する可能性があります。
- **実行中**: 待機時間が極端に伸びたり（時刻を戻した場合）、一瞬で終わったり（時刻を進めた場合）することがあります。
- **スケジュール**: 実行タイミングがズレたり、二重に実行されたり、実行がスキップされたりすることがあります。

**対策:** PCの時刻を変更した後は、設定画面で「保存」ボタンを押し直すか、アプリを再起動して設定を同期させることを強く推奨します。

### その他
- タスクスケジューラ登録はシステムに変更を加えます。
- 管理者権限がないとタスクの登録・削除に失敗する場合があります。
- 異常終了時はタスクが残存する可能性があるため、その場合は手動削除（cleanup.exe等）を行ってください。
